...
@TeleOp
//i like cheese
public class FINALFORM extends LinearOpMode {
    ...
    public static final double SHOULDER_INTAKE = 0.425;
    public static final double ELBOW_DRIVE= 0.5;
    public static final double ELBOW_INTAKE = 0.78;
    public static final double WRIST_DRIVE = 0.185; // measured at .255
    public static final double WRIST_INTAKE = 0.5049;
    ...
    public static final double SCORE_ZERO_SHOULDER = 0.945;
    public static final double SCORE_ZERO_WRIST = 0.3;
    public static final double SCORE_ZERO_ELBOW = 0.59;
    ...
    private void intakeFunction() {
        ...
        if (gamepad1.b && currentIntakeState == intakeState.IDLE) {
            activeIntakePosition = new IntakePosition(LIFT_DRIVE, SHOULDER_TOP_TWO, WRIST_TOP_TWO, ELBOW_TOP_TWO, MED_ACC, MED_VEL);
            currentIntakeState = intakeState.MOVING_SHOULDER;
        }
        // claw intake the next 2 from a stack of 3
        if (gamepad1.a && currentIntakeState == intakeState.IDLE) {
            activeIntakePosition = new IntakePosition(LIFT_DRIVE, SHOULDER_NEXT_TWO, WRIST_NEXT_TWO, ELBOW_NEXT_TWO, MED_ACC, MED_VEL);
            currentIntakeState = intakeState.MOVING_SHOULDER;
        }
    ...
    private DrivePosition activeDrivePosition = null;
    ...
    private ScorePosition activeScorePosition = null;
    ...
    private void handleScoreSequence(ScorePosition scorePos) {
    ...
    private void scoringFunction() {
        // scoring
        // score position one
        if (gamepad2.y && !gamepad2.left_bumper && (gamepad2.left_trigger < TRIGGER_THRESHOLD) && (currentScoreState == scoreState.IDLE)) {
            // Assign a new ScorePosition inside the if block
            activeScorePosition = new ScorePosition(SCORE_ONE_LIFT, SCORE_ONE_SHOULDER, SCORE_ONE_WRIST, SCORE_ONE_ELBOW, SUPER_ACC, SUPER_VEL);
            currentScoreState = scoreState.MOVING_SHOULDER;
        }
        // score position two
        if (gamepad2.b && !gamepad2.left_bumper && (gamepad2.left_trigger < TRIGGER_THRESHOLD) && (currentScoreState == scoreState.IDLE)) {
            // Assign a new ScorePosition inside the if block
            activeScorePosition = new ScorePosition(SCORE_TWO_LIFT, SCORE_TWO_SHOULDER, SCORE_TWO_WRIST, SCORE_TWO_ELBOW, SUPER_ACC, SUPER_VEL);
            currentScoreState = scoreState.MOVING_SHOULDER;
        }
        // score position three
        if (gamepad2.a && !gamepad2.left_bumper && (gamepad2.left_trigger < TRIGGER_THRESHOLD) && (currentScoreState == scoreState.IDLE)) {
            // Assign a new ScorePosition inside the if block
            activeScorePosition = new ScorePosition(SCORE_THREE_LIFT, SCORE_THREE_SHOULDER, SCORE_THREE_WRIST, SCORE_THREE_ELBOW, MED_ACC, MED_VEL);
            currentScoreState = scoreState.MOVING_SHOULDER;
        }
        // score position four
        //TODO: gotta put !gamepad2.left_bumper above
        if (gamepad2.x && !gamepad2.left_bumper && (gamepad2.left_trigger < TRIGGER_THRESHOLD) && (currentScoreState == scoreState.IDLE)) {
            // Assign a new ScorePosition inside the if block
            activeScorePosition = new ScorePosition(SCORE_FOUR_LIFT, SCORE_FOUR_SHOULDER, SCORE_FOUR_WRIST, SCORE_FOUR_ELBOW, MED_ACC, MED_VEL);
            currentScoreState = scoreState.MOVING_SHOULDER;
        }
        // score position five
        if ((gamepad2.left_bumper && gamepad2.y) && (gamepad2.left_trigger < TRIGGER_THRESHOLD) && (currentScoreState == scoreState.IDLE)) {
            // Assign a new ScorePosition inside the if block
            activeScorePosition = new ScorePosition(SCORE_FIVE_LIFT, SCORE_FIVE_SHOULDER, SCORE_FIVE_WRIST, SCORE_FIVE_ELBOW, MED_ACC, MED_VEL);
            currentScoreState = scoreState.MOVING_SHOULDER;
        }
        // score position six
        if ((gamepad2.left_bumper && gamepad2.b) && (gamepad2.left_trigger < TRIGGER_THRESHOLD) && (currentScoreState == scoreState.IDLE)) {
            // Assign a new ScorePosition inside the if block
            activeScorePosition = new ScorePosition(SCORE_SIX_LIFT, SCORE_SIX_SHOULDER, SCORE_SIX_WRIST, SCORE_SIX_ELBOW, MED_ACC, MED_VEL);
            currentScoreState = scoreState.MOVING_SHOULDER;
        }
        // score position seven
        if ((gamepad2.left_bumper && gamepad2.a) && (gamepad2.left_trigger < TRIGGER_THRESHOLD) && (currentScoreState == scoreState.IDLE)) {
            // Assign a new ScorePosition inside the if block
            activeScorePosition = new ScorePosition(SCORE_SEVEN_LIFT, SCORE_SEVEN_SHOULDER, SCORE_SEVEN_WRIST, SCORE_SEVEN_ELBOW, MED_ACC, MED_VEL);
            currentScoreState = scoreState.MOVING_SHOULDER;
        }
        // score position eight
        if ((gamepad2.left_bumper && gamepad2.x) && (gamepad2.left_trigger < TRIGGER_THRESHOLD) && (currentScoreState == scoreState.IDLE)) {
            // Assign a new ScorePosition inside the if block
            activeScorePosition = new ScorePosition(SCORE_EIGHT_LIFT, SCORE_EIGHT_SHOULDER, SCORE_EIGHT_WRIST, SCORE_EIGHT_ELBOW, MED_ACC, MED_VEL);
            currentScoreState = scoreState.MOVING_SHOULDER;
        }
        // score position nine
        if (((gamepad2.left_trigger > TRIGGER_THRESHOLD) && gamepad2.y) && !gamepad2.left_bumper && (currentScoreState == scoreState.IDLE)) {
            // Assign a new ScorePosition inside the if block
            activeScorePosition = new ScorePosition(SCORE_NINE_LIFT, SCORE_NINE_SHOULDER, SCORE_NINE_WRIST, SCORE_NINE_ELBOW, MED_ACC, MED_VEL);
            currentScoreState = scoreState.MOVING_SHOULDER;
        }
        // score position ten
        if (((gamepad2.left_trigger > TRIGGER_THRESHOLD) && gamepad2.b) && !gamepad2.left_bumper && (currentScoreState == scoreState.IDLE)) {
            // Assign a new ScorePosition inside the if block
            activeScorePosition = new ScorePosition(SCORE_TEN_LIFT, SCORE_TEN_SHOULDER, SCORE_TEN_WRIST, SCORE_TEN_ELBOW, MED_ACC, MED_VEL);
            currentScoreState = scoreState.MOVING_SHOULDER;
        }
        // score position eleven
        if (((gamepad2.left_trigger > TRIGGER_THRESHOLD) && gamepad2.a) && !gamepad2.left_bumper && (currentScoreState == scoreState.IDLE)) {
            // Assign a new ScorePosition inside the if block
            activeScorePosition = new ScorePosition(SCORE_ELEVEN_LIFT, SCORE_ELEVEN_SHOULDER, SCORE_ELEVEN_WRIST, SCORE_ELEVEN_ELBOW, MED_ACC, MED_VEL);
            currentScoreState = scoreState.MOVING_SHOULDER;
        }
